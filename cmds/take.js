...
module.exports = {
    ...
    exec: (client, msg, args) => {
        if (owners.isAdminOwner(msg.author.id)) {
            fetch('http://api.streamempires.live/users/shards/take', {method: 'put', body: JSON.stringify({authorization: process.env.SHARDKEY, shards: parseInt(args[1]), name: args[0]}), headers: {'Content-Type': 'application/json'}})
            .then(res => {
            ...
            .then(res => res.json())
            .then(json => msg.channel.createMessage(`The user \`${json.siteName}\` now has \`${json.shards}\` shards.`))
        }
    },

    options: {
        hidden: true,
        fullDescription: 'Remove shards from a user',
